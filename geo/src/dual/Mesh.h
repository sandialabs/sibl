...
class Poly
{
    ...
    unsigned int size(){return myNodes.size();}
...
{
    public:
    Mesh():maxPolySize(0){edgeUpToDate=false;}
...
class Dual: public Mesh
{
   ...
   private:
       ...
       double dist(Node A, Node B){return sqrt(pow(A.X()-B.X(),2.0)+pow(A.Y()-B.Y(),2.0)); }
       void walkB(Node A,Node &B, Node C);
       double absval(double a){if(a<0) return -a; else return a;}
        bool oblique(double ang){return (ang < 20 || ang > 160);}
