...
public:
    ...
    Node(std::tuple<double,double,double> pt,bool fringe)
    {
        x=std::get<XIND>(pt);
        y=std::get<YIND>(pt);
        z=std::get<ZIND>(pt);
    ...
    Node(std::tuple<double,double,double> pt,bool fringe,int id)
    {
        x=std::get<XIND>(pt);
        y=std::get<YIND>(pt);
        z=std::get<ZIND>(pt);
    ...
    Node(std::tuple<double,double> pt,bool fringe)
    {
        x=std::get<XIND>(pt);
        y=std::get<YIND>(pt);
        z=0;
    ...
    Node(std::tuple<double,double> pt,bool fringe,int id)
    {
        x=std::get<XIND>(pt);
        y=std::get<YIND>(pt);
        z=0;
    ...
    {
        return std::pow((x-A.x)*(x-A.x)+(y-A.y)*(y-A.y)+(z-A.z)*(z-A.z),0.5);
    }
    double magnitude()
    {
        return std::pow(x*x+y*y+z*z,0.5);
    }
    Node direction(const Node &A)
    {
        Node N;
        N.x = A.x-x;
        N.y = A.y-y;
