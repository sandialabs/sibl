...
namespace IntersectionPrimitives
{
	...
	{
	public:
		CartesianRay(double x0, double y0, CartesianDirection dir) : m_p(x0, y0), m_dir(dir) {}
		CartesianRay(std::pair<double, double> p, CartesianDirection dir) : m_p(p), m_dir(dir) {}
		CartesianRay(Point p, CartesianDirection dir) : m_p(std::move(p)), m_dir(dir) {}
	...
	{
	public:
		PolygonSegment(double p0x, double p0y, double p1x, double p1y) : m_p0(p0x, p0y), m_p1(p1x, p1y) {}
		PolygonSegment(Point p0, Point p1) : m_p0(std::move(p0)), m_p1(std::move(p1)) {}
	...
	{
	public:
		BoundingBox(double x0, double y0, double width, double height) : m_bottomLeft(x0, y0), m_topRight(x0 + width, y0 + height) {}
		BoundingBox(Point bottomLeft, double width, double height) : m_bottomLeft(std::move(bottomLeft)), m_topRight(m_bottomLeft.x() + width, m_bottomLeft.y() + height) {}
		BoundingBox(Point bottomLeft, Point topRight) : m_bottomLeft(std::move(bottomLeft)), m_topRight(std::move(topRight)) {}
